...
import { getJudge0LanguageId, getLanguageName } from "../libs/judge0.lib.js";
...
const executeRunCodeTestcases = asyncHandler(async (req, res) => {
    const { source_code, language_id, stdin, expected_outputs, problemId } =
        req.body;
    const userId = req.user.id;
    ...
    return res.status(200).json(
        new ApiResponse(
            200,
            {
                ...
                stderr: detailedTestCasesResults.some((r) => r.stderr)
                    ? JSON.stringify(
                          detailedTestCasesResults.map((r) => r.stderr),
                      )
                    : null,
                compileOutput: detailedTestCasesResults.some(
                    (r) => r.compile_output,
                ...
                memory: detailedTestCasesResults.some((r) => r.memory)
                    ? JSON.stringify(
                          detailedTestCasesResults.map((r) => r.memory),
                      )
                    : null,
                time: detailedTestCasesResults.some((r) => r.time)
                    ? JSON.stringify(
                          detailedTestCasesResults.map((r) => r.time),
                      )
                    : null,
                testCases: detailedTestCasesResults,
            },
            "Run Code Testcases execution successful",
        ),
    );
});

export { executeCode, executeRunCodeTestcases };
