    ...
    case "description":
      return (
        <div className="prose max-w-none space-y-8">
            ...
            <section>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {Object.entries(problem.examples).map(([lang, example]) => (
                  <div
                    key={lang}
                    className="rounded-xl bg-white dark:bg-neutral p-6 shadow-md border border-base-300"
                  >
                    <div className="text-sm font-semibold text-primary mb-2 uppercase tracking-wider">
                      Language: {lang}
                    </div>
                    ...
                    <div className="mb-3">
                      <div className="text-accent mb-1 font-semibold">Input:</div>
                      <pre className="bg-base-200 px-4 py-2 rounded-md text-sm whitespace-pre-wrap">
                        {example.input}
                      </pre>
                    </div>
                    ...
                    <div className="mb-3">
                      <div className="text-accent mb-1 font-semibold">Output:</div>
                      <pre className="bg-base-200 px-4 py-2 rounded-md text-sm whitespace-pre-wrap">
                        {example.output}
                      </pre>
                    </div>
          ...
          {problem.constraints && (
            <section>
              <h3 className="text-xl font-bold mb-4 text-primary">Constraints</h3>
              <div className="bg-base-200 p-4 rounded-xl border border-base-300 text-base-content/90 text-sm font-mono">
                {problem.constraints}
              </div>
            </section>
      ...
      return (
        <div className="p-4">
          {problem?.hints ? (
            ...
            <div className="text-center text-base-content/60">No hints available</div>
          )}
        </div>
      );
...
return (
  <div className="min-h-screen bg-gradient-to-b from-white to-base-100 dark:from-gray-900 dark:to-base-200">
    <nav className="sticky top-0 z-50 bg-base-100/80 backdrop-blur shadow px-6 py-3 flex justify-between items-center border-b border-base-300">
      <div className="flex items-center gap-2">
        <Link to="/" className="flex items-center text-primary font-semibold text-lg">
          <Home className="w-5 h-5 mr-1" /> XCodeLab
        </Link>
        <ChevronRight className="w-4 h-4 text-base-content/40 mx-2" />
        <span className="text-sm font-medium text-base-content/70">{problem.title}</span>
      </div>
    ...
    <div className="container mx-auto px-4 py-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-2xl bg-base-100 shadow border border-base-300">
          <div className="border-b border-base-300 px-6 pt-4 flex gap-4">
            {["description", "submissions", "discussion", "hints"].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`tab font-medium capitalize flex items-center gap-2 py-2 px-4 ${
                  activeTab === tab ? "tab-active text-primary border-b-2 border-primary" : "text-base-content/70"
                }`}
              >
        ...
        <div className="rounded-2xl bg-base-100 shadow border border-base-300">
          <div className="border-b border-base-300 px-6 pt-4">
            <div className="tab tab-active flex items-center gap-2 text-primary font-medium">
              <Terminal className="w-4 h-4" /> Code Editor
            </div>
          </div>
          ...
          <div className="flex justify-between items-center gap-4 px-6 py-4 border-t border-base-300 bg-base-200">
            <button
              className={`btn btn-primary gap-2 ${isExecuting ? "loading" : ""}`}
              onClick={handleRunCode}
              disabled={isExecuting}
      ...
      <div className="rounded-2xl bg-base-100 shadow border border-base-300 mt-8">
        <div className="card-body">
          {submission ? (
            <Submission submission={submission} />
          ) : (
            <>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-semibold text-base-content">Test Cases</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="table w-full">
                  <thead>
                    <tr>
                      <th className="text-sm font-semibold">Input</th>
                      <th className="text-sm font-semibold">Expected Output</th>
                    </tr>
                  </thead>
                  <tbody>
                    {testcases.map((testCase, index) => (
